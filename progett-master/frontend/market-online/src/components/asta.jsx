...
import './asta.css';
import { addToCartAsta } from '../utils/APIRoutes';
...
function Asta() {
  ...
  const handleAddToCart = async (id, astaId) => {
    try {
        // Chiamata alla funzione addToCart del backend
        const response = await fetch(addToCartAsta(id, astaId, 1), { method: 'POST' }); // Assuming 1 as the quantity, adjust accordingly
  ...
  return (
    <div>
      <form  className="codiceAsta" onClick={handleSearch}>
        ...
        {aste.map((asta, index) => (
          <div className='AstaContainer' key={index} style={{ marginRight: '10px' }}>
            ...
            <button onClick={() => {
              ...
              handleAddToCart(userid, asta._id);
            }}>
            Riscatta
          </button>
